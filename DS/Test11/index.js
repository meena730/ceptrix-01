(function () {
  ...
  const check = setInterval(() => {
    ...
    let subtotal = 0;
    rows.forEach((cell) => { const label = cell .closest("tr") ?.querySelector("th") ?.textContent?.trim() .toLowerCase();
    ...
    if (!subtotal) return clearInterval(check);
    clearInterval(check);
    ...
    const barHTML = `
      <div class="free-shipping-container">
        ...
        <div class="bar-track">
          ...
          <div class="bar-badge">
            <img src="https://res.cloudinary.com/diwhc4afs/image/upload/v1753178106/Vector_1_yhoima.png" />
          </div>
        </div>
      </div>
    `;
    ...
    const target =
      document.querySelector("#delivery-widget") ||
      document.querySelector(".gmd-product-bought");
    ...
    if (target) {
      ...
      setTimeout(() => {
        ...
        const badge = wrapper.querySelector(".bar-badge");
